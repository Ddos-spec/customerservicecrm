...
function initializeGatewayApi(deps) {
    ...
    /**
     * POST /api/v1/gateway/send-document
     * Send an document via WhatsApp Gateway and log it.
     * Tenant can be resolved by body tenant_id OR header X-Tenant-Key.
     *
     * Body:
     * {
    ...
    router.post('/send-document', async (req, res) => {
        try {
            const { tenant_id, phone_number } = req.body;
            const documentSource = (req.body?.document_url || req.body?.document || '').toString().trim();
            const caption = (req.body?.caption || '').toString();
            const viewOnceInput = typeof req.body?.view_once !== 'undefined' ? req.body.view_once : req.body?.viewonce;
            const viewOnce = parseBooleanFlag(viewOnceInput, false);

            if (!phone_number || !documentSource) {
                return res.status(400).json({
                    success: false,
                    error: 'Missing fields: phone_number, document_url'
                });
...
function initializeGatewayApi(deps) {
    ...
    /**
     * POST /api/v1/gateway/send-video
     * Send an video via WhatsApp Gateway and log it.
     * Tenant can be resolved by body tenant_id OR header X-Tenant-Key.
     *
     * Body:
     * {
    ...
    router.post('/send-video', async (req, res) => {
        try {
            const { tenant_id, phone_number } = req.body;
            const videoSource = (req.body?.video_url || req.body?.video || '').toString().trim();
            const caption = (req.body?.caption || '').toString();
            const viewOnceInput = typeof req.body?.view_once !== 'undefined' ? req.body.view_once : req.body?.viewonce;
            const viewOnce = parseBooleanFlag(viewOnceInput, false);

            if (!phone_number || !videoSource) {
                return res.status(400).json({
                    success: false,
                    error: 'Missing fields: phone_number, video_url'
                });
...
function initializeGatewayApi(deps) {
    ...
    /**
     * POST /api/v1/gateway/send-audio
     * Send an audio via WhatsApp Gateway and log it.
     * Tenant can be resolved by body tenant_id OR header X-Tenant-Key.
     *
     * Body:
     * {
     *   "tenant_id": "optional-tenant-uuid",
     *   "phone_number": "628123456789",
     *   "audio_url": "https://example.com/audio.jpg",
     *   "caption": "Optional caption",
     *   "view_once": false
     * }
     */
    router.post('/send-audio', async (req, res) => {
        try {
            const { tenant_id, phone_number } = req.body;
            const audioSource = (req.body?.audio_url || req.body?.audio || '').toString().trim();
            const caption = (req.body?.caption || '').toString();
            const viewOnceInput = typeof req.body?.view_once !== 'undefined' ? req.body.view_once : req.body?.viewonce;
            const viewOnce = parseBooleanFlag(viewOnceInput, false);

            if (!phone_number || !audioSource) {
                return res.status(400).json({
                    success: false,
                    error: 'Missing fields: phone_number, audio_url'
                });
            }
