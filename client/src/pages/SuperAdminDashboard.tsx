...
import { TrendingUp, Users, Server, Activity, DollarSign, ArrowRight, Smartphone, Terminal, Settings, Trash2, RefreshCw, ShieldCheck, QrCode, ArrowLeft, Key, Globe, Eye, Plus, WifiOff } from 'lucide-react';
  ...
  return (
    <div>
      ...
      <div className="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden mb-8">
        {/* Header Control Center - Green Theme */}
        ...
        <div className="p-8">
              ...
              <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
                 <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div className="space-y-6">
                       ...
                       <div className="bg-green-50/50 border border-green-100/50 rounded-3xl p-6">
                          ...
                          <div className="space-y-4">
                             <div>
                                ...
                                <div className="flex space-x-2">
                                   <input type="password" readOnly value={selectedSession.apiKey} className="flex-1 bg-white border border-green-100 rounded-xl px-4 py-2.5 text-xs font-mono text-green-600" />
                                   <button className="p-2.5 bg-white border border-green-100 text-green-600 rounded-xl hover:bg-green-50"><Eye size={16} /></button>
                                </div>
                             </div>
                             <button className="w-full py-2.5 bg-green-100 text-green-700 text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-green-200 transition-colors">Generate Token Baru</button>
                          </div>
                       </div>
                    </div>
                       ...
                       <div className="bg-white border border-gray-100 rounded-3xl p-8 shadow-sm">
                          ...
                          <div className="mt-10 pt-8 border-t border-gray-50">
                             <div className="flex items-center justify-between mb-4">
                                ...
                                <button className="text-[10px] font-black text-green-600 flex items-center space-x-1 hover:underline">
                                   <Plus size={12} />
                             ...
                             <div className="space-y-3">
                                <div className="flex space-x-2">
                                   <input type="text" defaultValue="https://main-crm.com/api/wa-hook" className="flex-1 bg-gray-50 border border-gray-200 rounded-2xl px-5 py-3 text-sm font-medium text-gray-700 focus:outline-none focus:border-green-500" />
                                   <button className="p-3 text-red-400 hover:bg-red-50 rounded-2xl transition-colors"><Trash2 size={18} /></button>
                                </div>
                                <div className="flex space-x-2">
                                   <input type="text" defaultValue="https://n8n.workflow.io/webhook/123-abc" className="flex-1 bg-gray-50 border border-gray-200 rounded-2xl px-5 py-3 text-sm font-medium text-gray-700 focus:outline-none focus:border-green-500" />
                                   <button className="p-3 text-red-400 hover:bg-red-50 rounded-2xl transition-colors"><Trash2 size={18} /></button>
                                </div>
                             </div>
                             <button className="mt-6 w-full py-4 bg-emerald-900 text-white font-black uppercase tracking-widest text-xs rounded-[1.5rem] shadow-xl shadow-emerald-100 hover:bg-emerald-800 transition-all active:scale-95">Simpan Konfigurasi Mesin</button>
                          </div>
                       </div>

                       {/* Mini Log for this session */}
                       <div className="bg-slate-900 rounded-3xl p-6 font-mono text-[10px] border border-slate-800 shadow-2xl">
                         ...
                         <div key={session.id} className="bg-white border border-gray-100 rounded-[2.5rem] p-6 hover:shadow-2xl hover:shadow-green-100/50 transition-all relative group">
                            ...
                            <div className="flex space-x-3">
                               <button 
                                 ...
                                 className="flex-1 py-3.5 bg-emerald-900 text-white text-[10px] font-black uppercase tracking-widest rounded-2xl hover:bg-emerald-800 transition-all shadow-lg shadow-emerald-100"
                               >
                                  Lihat Detail
                               </button>
                               <button 
           ...
           {!selectedSession && activeTab === 'logs' && (
              <div className="animate-in fade-in duration-500">
                 ...
                 <div className="bg-slate-900 rounded-3xl p-6 font-mono text-[11px] h-[450px] overflow-y-auto shadow-2xl border border-slate-800 custom-scrollbar">
                    {logs.map((log, idx) => (
                    ...
                    <div className="mt-4 text-green-500/50 animate-pulse flex items-center space-x-2">
              ...
              <div className="max-w-3xl animate-in fade-in duration-500">
                 ...
                 <div className="space-y-8">
                    <div className="group">
                       ...
                       <div className="flex space-x-3">
                          <input type="text" defaultValue="https://api.customerservice.com/webhooks/whatsapp" className="flex-1 bg-gray-50 border-2 border-gray-100 rounded-2xl px-6 py-4 text-sm font-mono text-green-600 focus:outline-none focus:border-green-500 focus:bg-white transition-all shadow-inner" />
                          <button className="px-8 py-4 bg-green-600 text-white rounded-2xl font-black uppercase tracking-widest text-[10px] hover:bg-green-700 shadow-xl shadow-green-100 transition-all active:scale-95">Update Inti</button>
                       </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                       <div className="bg-gray-50 p-6 rounded-3xl border border-gray-100 group hover:border-green-100 transition-all">
                          <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-3 group-hover:text-green-600">Batas Konkurensi Maks</label>
                          <div className="flex items-center space-x-4">
                             <input type="number" defaultValue={50} className="w-24 bg-white border border-gray-200 rounded-xl px-4 py-2.5 font-bold text-gray-700 focus:outline-none focus:border-green-500" />
                             <span className="text-xs text-gray-500 font-medium">Instance paralel</span>
                          </div>
                       </div>
                       <div className="bg-gray-50 p-6 rounded-3xl border border-gray-100 group hover:border-green-100 transition-all">
                          <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-3 group-hover:text-green-600">Kebijakan TTL Redis (Hari)</label>
                          <div className="flex items-center space-x-4">
                             <input type="number" defaultValue={30} className="w-24 bg-white border border-gray-200 rounded-xl px-4 py-2.5 font-bold text-gray-700 focus:outline-none focus:border-green-500" />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 bg-white p-8 rounded-[2.5rem] border border-gray-100 shadow-sm">
            <div className="flex justify-between items-center mb-8">
              ...
              <button className="text-xs font-black uppercase tracking-widest text-green-600 hover:text-green-700">Lihat Audit Trail</button>
            </div>
            <div className="space-y-4">
              {[1, 2, 3].map((i) => (
                <div key={i} className="flex items-center justify-between p-5 hover:bg-gray-50 rounded-[1.5rem] transition-all border border-gray-50 group">
                  <div className="flex items-center space-x-5">
                    <div className="w-14 h-14 rounded-2xl bg-green-50 flex items-center justify-center text-green-600 font-black text-lg group-hover:scale-110 transition-transform">
                      T{i}
                    </div>
                    <div>
...
const TabButton = ({ active, onClick, icon, label }: any) => (
  <button 
    onClick={onClick}
    className={`flex items-center space-x-2 px-5 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${active ? 'bg-emerald-700 text-white shadow-lg' : 'text-emerald-200 hover:text-white hover:bg-emerald-700/50'}`}
  >
    {icon}
    <span>{label}</span>
  </button>
...
const ToggleItem = ({ label, desc, active }: any) => (
  <div className="flex items-center justify-between p-4 bg-gray-50 rounded-2xl border border-gray-100 group hover:border-green-100 transition-all">
     ...
     <div className={`w-10 h-6 rounded-full relative transition-colors cursor-pointer ${active ? 'bg-green-600' : 'bg-gray-300'}`}>
...
const HealthItem = ({ label, status }: any) => (
  <div className="flex items-center justify-between group">
    <span className="text-gray-500 text-xs font-black uppercase tracking-widest group-hover:text-green-600 transition-colors">{label}</span>
